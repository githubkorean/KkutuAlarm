...
(function() {
    ...
    function autoStartGame() {
        ...
        // 방장의 상태 확인
        const master = document.querySelector('.room-user-ready.room-user-master');
        let readyUsers = document.querySelectorAll('.room-user-ready.room-user-readied').length; // 준비 완료된 인원 수

        // 대기 상태의 유저가 있을 경우 준비 인원을 0명으로 설정
        if (document.querySelector('.room-user-ready') && document.querySelector('.room-user-ready').innerText.includes('대기')) {
            console.log("대기 상태 유저가 있습니다. 준비 인원을 0명으로 처리합니다.");
            readyUsers = 0; // 대기 상태 유저가 있을 때는 준비 인원을 0으로 설정
        }

        // 방장이 관전 상태일 경우 준비 인원에서 -1
        if (master && master.classList.contains('room-user-ready') && master.innerText.includes('관전')) {
            console.log("방장이 관전 중입니다. 준비 인원에서 -1 처리합니다.");
            readyUsers--; // 방장이 관전 중이므로 준비 인원에서 -1
        }

        console.log(`자동 시작 조건 확인: 준비 인원 - ${readyUsers}, 시작 버튼 표시 - ${startBtn.style.display}`);

        // 시작 버튼이 표시되고, 준비 인원이 1명 이상일 때만 시작
        if (readyUsers >= 1 && startBtn.style.display === 'block') {
