...
(function() {
    ...
    function getUserCounts() {
        ...
        document.querySelectorAll('.room-user-ready').forEach((user) => {
            const userName = user.querySelector('.room-user-name')?.textContent;

            if (user.classList.contains('room-user-master')) {
                if (user.textContent.includes('관전')) {
                    spectatorsCount++; // 방장이 관전 중이라면 관전 인원에 추가
                } else {
                    readyUserCount++; // 방장이 준비 중일 경우
                }
            } else if (user.textContent.includes('준비') && !isBot(userName)) {
                readyUserCount++;
        ...
        // 봇이 있다면 준비 인원에서 1명으로 취급
        if (readyUserCount > 0) {
            readyUserCount = Math.max(0, readyUserCount - 1); // 최소 0으로 유지
        }
        ...
        return { readyUserCount, waitingCount, spectatorsCount };
    }
